...
def obter_centroides():
    ...
    gdf_centroides = gpd.GeoDataFrame(gdf_population_cachoeiro_de_itapemirim[['centroid']].copy(), geometry='centroid')  # Usar 'geometry' para especificar a coluna de geometria
...
def obter_rotas_centroide_para_educacao():
    ...
    centroides = gpd.read_file(
        caminho_geojson_population_cachoeiro_de_itapemirim_centroides)
    unidades_saude = gpd.read_file(
        caminho_geojson_unidades_ensino_cachoeiro_de_itapemirim)
    ...
    for idx, row in centroides.iterrows():
        ...
        unidade_saude_mais_proxima = unidades_saude.geometry.distance(
            centroid).idxmin()
        unidade_saude = unidades_saude.loc[unidade_saude_mais_proxima].geometry

        # Encontrar os nós mais próximos no grafo viário
        nodo_origem = ox.distance.nearest_nodes(
            grafo_viario, centroid.x, centroid.y)
        nodo_destino = ox.distance.nearest_nodes(
            grafo_viario, unidade_saude.x, unidade_saude.y)
...
def obter_gdf_peso_hexagonos():
    ...
    unidades_saude = gpd.read_file(caminho_geojson_unidades_ensino_cachoeiro_de_itapemirim)
