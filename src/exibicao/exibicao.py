...
def criar_mapa():
    ...
    gdf_unidades_ensino_cachoeiro_de_itapemirim = gpd.read_file(caminho_geojson_unidades_ensino_cachoeiro_de_itapemirim)
    ...
    gdf_unidades_ensino_cachoeiro_de_itapemirim = gdf_unidades_ensino_cachoeiro_de_itapemirim.to_crs(epsg=4674)
    ...
    if not gdf_unidades_ensino_cachoeiro_de_itapemirim.empty:
        for idx, row in gdf_unidades_ensino_cachoeiro_de_itapemirim.iterrows():
            if row.geometry.geom_type == 'Point':  # Verificar se a geometria é um ponto
                folium.Marker(location=[row.geometry.y, row.geometry.x], 
                              popup=Popup(f'Unidade de Ensino: {row["nome"]}'), 
...
def obter_app_ui():
    ...
    app_ui = ui.page_fluid(
        ...
        ui.tags.div(
            ...
            ui.tags.h2("Mapeamento Geoespacial de Acesso às Escolas Públicas de Ensino Fundamental",
